...
export default function WorkoutPage() {
  ...
  // Start gate — user must tap "Start Workout" before the session begins
  ...
  useEffect(() => {
    async function load() {
      ...
      // Load workout_exercises then exercises separately (avoids join fragility)
      ...
      const exerciseIds = [...new Set(weRows.map((w: { exercise_id: string }) => w.exercise_id).filter(Boolean))] as string[]
      const exerciseMap = new Map<string, ExRow>()
      if (exerciseIds.length > 0) {
        const { data: exData } = await supabase.from('exercises').select('id, name, video_url, muscle_group').in('id', exerciseIds)
  ...
  async function finishWorkout() {
    ...
    if (completedSets.length > 0) await supabase.from('workout_sets').insert(completedSets)
          ...
          <div className="bg-white rounded-2xl shadow-sm border border-gray-100 overflow-hidden">
            ...
            <div className="p-4">
              <div className="grid grid-cols-[28px_1fr_1fr_40px] gap-2 mb-2 px-1 text-xs text-gray-400 font-medium">
                <span>#</span><span>Reps</span><span>Weight</span><span className="text-center">✓</span>
              </div>
